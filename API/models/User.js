...
class User{
    ...
    async findById(id){
        try {
            const columns = [
                "id", "email", "role", "name", 'ra', 'course'
            ];
            var result = await knex.select(columns).where({id:id}).table("users");
    ...
    async new(email,password,name,course,ra,role){
        try{
            var hash = await bcrypt.hash(password, 10);
            await knex.insert({email,password: hash,name,role: role,course,ra}).table("users");
        }catch(err){
